...
from v5_final.s11_v2_execution_readiness_v9 import (
    DECISION,
    OUTPUT,
    S11V2ExecutionReadinessV9Error,
    _digest,
    _embedded_digest,
    _load,
...
def test_post_item023_checkpoint_or_frozen_readiness_is_valid() -> None:
    if OUTPUT.exists():
        ...
        assert evidence["observed"]["terminal_count"] == 24
        assert evidence["observed"]["next_queue_index"] == 24
...
def test_post_verification_storage_is_fail_closed(monkeypatch) -> None:
    from v5_final import s11_v2_execution_readiness_v9 as subject
    ...
    with pytest.raises(S11V2ExecutionReadinessV9Error, match="fell below 40 GiB"):
        _require_minimum_free_storage()
...
def test_capture_refuses_dirty_tree(monkeypatch, tmp_path: Path) -> None:
    ...
    with pytest.raises(S11V2ExecutionReadinessV9Error, match="clean worktree"):
        subject.capture()
