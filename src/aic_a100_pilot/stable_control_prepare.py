"""Outcome-free source preparation for the stable-control successor."""
...
def prepare_bundle(
    ...
    if bundle_path.exists() or manifest_path.exists():
        raise A100PilotError("refusing to overwrite stable-control preparation")
    ...
    if not expected_head or git("rev-parse", "HEAD") != expected_head:
        raise A100PilotError("stable-control source preparation Git HEAD differs")
    observed_sources = {
        path.relative_to(ROOT).as_posix(): sha256_file(path)
        for path in SOURCE_PATHS
    }
    if observed_sources != contract["source_binding"]:
        raise A100PilotError("stable-control source differs from frozen contract")
    ...
    manifest = {
        "schema": "aic-a100-pilot.stable-control-prepared-source.v1",
        "alias": alias,
