"""Reconcile the item-022 incident with its formal cap-rejection successor."""
...
OUTPUT_DIR = (
    ROOT / "artifacts/v5-final/parent-native/s11-v2-item022-terminal-reconciliation-v1"
)
OUTPUT = OUTPUT_DIR / "terminal-reconciliation-v1.json"
READINESS_V6 = (
    ROOT / "artifacts/v5-final/parent-native/s11-v2-execution-readiness-v6"
...
QUEUE_INDEX = 22
DECISION = "RECONCILE_S11_V2_ITEM022_INCIDENT_TO_FORMAL_CAP_REJECTION"
SOURCE_PATHS = (
...
class S11V2Item022TerminalReconciliationError(RuntimeError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise S11V2Item022TerminalReconciliationError(f"invalid JSON: {path}") from error
    if not isinstance(value, dict) or raw != canonical_json_bytes(value):
        raise S11V2Item022TerminalReconciliationError(f"noncanonical JSON: {path}")
    return value
...
def _remote_head(branch: str) -> str:
    ...
    if not line:
        raise S11V2Item022TerminalReconciliationError("remote branch is absent")
    return line.split()[0]


def historical_artifact_valid(
    path: Path, artifact: Mapping[str, Any], *, digest_field: str, decision: str
) -> bool:
    commit = artifact.get("repository_head") or artifact.get(
        "captured_repository_state", {}
    ).get("local_head")
    manifest = (
        artifact.get("bindings", {}).get("source_sha256")
        or artifact.get("binding", {}).get("source_sha256")
        or {}
    )
    return (
        artifact_is_immutable_git_blob(path)
        and _embedded_digest(artifact, digest_field)
        ...
        and (
            not manifest
            or manifest_matches_commit(
                [{"path": key, "sha256": value} for key, value in manifest.items()],
                commit,
            )
        )
    )
...
def inspect_terminal_reconciliation() -> dict[str, Any]:
    ...
    checks = {
        ...
        "historical_readiness_v6_valid_at_captured_commit": historical_artifact_valid(
            READINESS_V6,
            readiness,
        ...
        "attempt2_is_zero_work_preverifier_rejection": cap_event["payload"]["operation"]
        == "cap-rejection"
        ...
        "terminal_prefix_advanced_to_23": len(prefix) == 23
        and prefix[-1]["receipt_digest"] == receipt["receipt_digest"]
        and progress["terminal_count"] == 23
    ...
    if failures:
        raise S11V2Item022TerminalReconciliationError(failures)
    terminal_paths = (
        RETRY_DISPATCH,
        *(RAW_ROOT / f"{index:08d}-{kind}.json" for index, kind in (
            (5, "attempt-start"), (6, "kernel-event"), (7, "terminal")
        )),
        RESULT,
        RECEIPT,
        PROGRESS,
    ...
    return {
        ...
        "bindings": {
            ...
            "readiness_v6_sha256": _sha(READINESS_V6),
            "readiness_v6_digest": readiness["readiness_digest"],
...
def capture() -> dict[str, Any]:
    if OUTPUT.exists():
        raise S11V2Item022TerminalReconciliationError(
    ...
    body = {
        "schema": "v5-final.s11-v2-item022-terminal-reconciliation.v1",
        "stage": "PHASE_C_ITEM022_FORMAL_CAP_REJECTION_TERMINAL",
        "status": DECISION,
        "decision": DECISION,
        "repository_head": head,
        **inspect_terminal_reconciliation(),
        "scientific_interpretation": (
            "Infrastructure cap rejection before Verifier V2 or molecular outcome work; "
            "not a method-performance observation."
        ),
        "authorization": {
            "item023_and_later": "NOT_AUTHORIZED_PENDING_ADDITIVE_READINESS_SUCCESSOR",
...
def audit_frozen() -> dict[str, Any]:
    ...
    checks = {
        ...
        "terminal_evidence_unchanged": all(
            _sha(ROOT / path) == expected
            for path, expected in artifact["bindings"]["terminal_evidence_sha256"].items()
        ),
        "sources_current": all(
            _sha(ROOT / path) == expected
            for path, expected in artifact["bindings"]["source_sha256"].items()
        ),
        "continuation_closed": artifact["authorization"]["item023_and_later"]
