"""Authorize one outcome-free retry of the rolled-back S11-v2 item 022.

The authorization is additive.  It preserves the incident session exactly and
reconstructs the corrected symbolic bound from its frozen, pre-outcome
candidate descriptors.  No molecular runtime, generator, state, energy,
optimizer, or FCI service is opened while this gate is inspected.
"""
...
QUEUE_INDEX = 22
DECISION = "AUTHORIZE_S11_V2_ITEM022_SAME_ITEM_PREVERIFIER_CAP_RETRY"
SOURCE_PATHS = (
...
class S11V2Item022RetryAuthorizationError(RuntimeError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise S11V2Item022RetryAuthorizationError(f"invalid JSON: {path}") from error
    if not isinstance(value, dict) or raw != canonical_json_bytes(value):
        raise S11V2Item022RetryAuthorizationError(f"noncanonical JSON: {path}")
    return value
...
def _remote_head(branch: str) -> str:
    ...
    if not line:
        raise S11V2Item022RetryAuthorizationError("remote branch is absent")
    return line.split()[0]
...
def _selected_descriptor_costs(
    ...
    if not isinstance(descriptors, list) or not isinstance(selected, list):
        raise S11V2Item022RetryAuthorizationError("candidate freeze is malformed")
    selected_ids = tuple(selected)
    if not selected_ids or len(set(selected_ids)) != len(selected_ids):
        raise S11V2Item022RetryAuthorizationError("selected candidate IDs differ")
    by_id = {value.get("candidate_id"): value for value in descriptors}
    if len(by_id) != len(descriptors) or any(value not in by_id for value in selected_ids):
        raise S11V2Item022RetryAuthorizationError("candidate descriptors differ")
    costs: list[int] = []
    for candidate_id in selected_ids:
        ...
        if not isinstance(source, list) or not isinstance(target, list):
            raise S11V2Item022RetryAuthorizationError("relation arity is absent")
        costs.append(
            symbolic_check_cost_from_arity(
...
def inspect_retry_readiness() -> dict[str, Any]:
    ...
    replay = replay_raw_ledger(
        RAW_ROOT, request=request.work_request, cap=request.outcome_cap,
    ...
    predicted = relation_aware_symbolic_upper_bound(
        candidate_count=candidate_count, selected_costs=selected_costs
    )
    cap = int(request.item["verifier_componentwise_cap"]["N_symbolic_checks"])
    evidence_paths = (
        DISPATCH,
        SESSION_BINDING,
        TOP_K_FREEZE,
        *tuple(sorted(CHECKPOINT_ROOT.glob("numeric-*.json"))),
    )
    checks = {
        "incident_is_immutable_formal_no_go": all(audit_incident()["checks"].values())
        and incident["decision"]
        == "NO_GO_S11_V2_ITEM022_NONCONSERVATIVE_SYMBOLIC_PRECHECK",
        ...
        "selection_identity_unchanged": selected_ids
        == tuple(top_k["selected_candidate_ids"])
        and len(selected_ids) == int(request.item["verifier_policy"]["top_k"]),
        "relation_aware_bound_reconstructs_incident": list(selected_costs)
        == incident["observed"]["selected_symbolic_check_counts"]
        and candidate_count == incident["observed"]["candidate_descriptor_count"]
        and predicted == incident["observed"]["reconstructed_symbolic_work"],
        "unchanged_cap_rejects_before_verifier": predicted > cap
        and cap == incident["observed"]["frozen_symbolic_precheck"],
        "outcome_free_zero_dense_and_fci": asdict(replay.work_total)["energy_evaluations"] == 0
    ...
    if failures:
        raise S11V2Item022RetryAuthorizationError(failures)
    ...
    return {
        ...
        "observed": {
            ...
            "historical_fixed_cost_assumption": candidate_count + 5 * len(selected_ids),
        ...
        "bindings": {
            ...
            "item022_incident_sha256": _sha(ITEM022_INCIDENT),
            "item022_incident_digest": incident["incident_digest"],
...
def capture() -> dict[str, Any]:
    if OUTPUT.exists():
        raise S11V2Item022RetryAuthorizationError("retry authorization already exists")
    branch = _git("branch", "--show-current")
    head = _git("rev-parse", "HEAD")
    if _git("status", "--porcelain"):
        raise S11V2Item022RetryAuthorizationError("capture requires a clean worktree")
    if head != _remote_head(branch):
        raise S11V2Item022RetryAuthorizationError("local and remote heads differ")
    adapter = QueueV2NativeAdapter()
    evidence = inspect_retry_readiness()
    body = {
        "schema": "v5-final.s11-v2-item022-same-item-retry-authorization.v1",
        "stage": "PHASE_C_ITEM022_PREVERIFIER_RETRY_AUTHORIZATION",
        "status": DECISION,
        "decision": DECISION,
        ...
        **evidence,
        "authorization": {
            "item022_retry": "AUTHORIZED_ONCE_APPEND_ONLY_SAME_CAP_PREVERIFIER_REJECTION",
            "item023_and_later": "NOT_AUTHORIZED_PENDING_READINESS_V6",
        ...
        "semantic_diff": {
            ...
            "cap_changed": False,
            "counter_reset": False,
            "correction": "Derive selected relation symbolic costs from frozen arity metadata.",
        },
    }
...
def audit_frozen(*, require_live: bool = False) -> dict[str, Any]:
    ...
    checks = {
        "authorization_digest_valid": _embedded_digest(artifact, "authorization_digest"),
        ...
        "incident_unchanged": bindings["item022_incident_sha256"] == _sha(ITEM022_INCIDENT),
        ...
        and artifact["authorization"]["item022_retry"]
