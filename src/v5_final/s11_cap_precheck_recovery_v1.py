"""Append-only terminal recovery for the S11 item-022 cap-precheck incident."""
...
QUEUE_INDEX = 22
ITEM_KEY = "022-a40e5a3b43f7a6851b4aae648ebd2899034f36ad46df71ed9bc0603d85078c18"
...
RECOVERY_SOURCES = (
    ROOT / "src/v5_final/s11_cap_precheck_recovery_v1.py",
    ROOT / "tests/test_v5_final_s11_cap_precheck_recovery_v1.py",
    ROOT / ".github/workflows/v5-s11-cap-precheck-recovery-gate.yml",
)
EXPECTED_OPERATIONS = ("full-physical-resource-recount", "statevector-recomputation")


class S11CapPrecheckRecoveryError(RuntimeError):
    pass
...
def _canonical(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise S11CapPrecheckRecoveryError(f"invalid JSON: {path}") from error
    if not isinstance(value, dict) or raw != canonical_json_bytes(value):
        raise S11CapPrecheckRecoveryError(f"noncanonical JSON: {path}")
    return value
...
def _item() -> dict[str, Any]:
    ...
    if (
        ...
        != "development-queue-item-v4:a40e5a3b43f7a6851b4aae648ebd2899034f36ad46df71ed9bc0603d85078c18"
        or item.get("method_id") != "v5-fixed-source-whitelist-no-replenishment"
        or item.get("case_id") != "h6-1.5"
        or item.get("work_envelope") != "LOW"
    ):
        raise S11CapPrecheckRecoveryError("frozen item-022 identity drifted")
    return item
...
def _request_cap() -> tuple[Any, WorkDelta]:
    ...
    if work_cap_digest(cap) != item["work_cap_digest"]:
        raise S11CapPrecheckRecoveryError("frozen work cap drifted")
    return request, cap


def _projected_binding_delta() -> WorkDelta:
    binding = _item()["candidate_work_binding"]
    body = dict(binding)
    observed = body.pop("binding_digest", None)
    if observed != _digest(body):
        raise S11CapPrecheckRecoveryError("candidate binding digest mismatch")
    return WorkDelta(
...
def _embedded_state(declaration: Mapping[str, Any]) -> tuple[Any, dict[str, Any]]:
    ...
    with tempfile.TemporaryDirectory(prefix="s11-cap-precheck-") as temporary:
...
def build_declaration() -> dict[str, Any]:
    ...
    checkpoint = services._read_outcome_checkpoint(CHECKPOINT_PATH, request)
    projected = _add(state.work_total, _projected_binding_delta())
    exceeded = [
    ...
    if (
        ...
        raise S11CapPrecheckRecoveryError("local item-022 incident differs from declaration")
    declaration: dict[str, Any] = {
        "schema": "v5-final.s11-cap-precheck-incident-declaration.v1",
        ...
        "deterministic_precheck": {
            ...
            "projected_delta": asdict(_projected_binding_delta()),
...
def audit_declaration(record: Mapping[str, Any] | None = None) -> dict[str, bool]:
    ...
    checks = {
        ...
        and declaration["deterministic_precheck"]["exceeded_components"] == exceeded,
        "candidate_energy_zero": all(
            event.operation != "candidate-energy-evaluation" for event in state.work_events
        ) and declaration["observed_state"]["candidate_energy_evaluations"] == 0,
        "append_only_no_kernel_policy": declaration.get("recovery_policy") == {
            "append_only": True,
            "preserve_all_four_records": True,
            "reuse_existing_checkpoint": True,
            "append_exact_zero_delta_cap_rejection": True,
    ...
    if failures:
        raise S11CapPrecheckRecoveryError("declaration audit failed: " + ", ".join(failures))
    return checks


def build_static_report() -> dict[str, Any]:
    report = {
        "schema": "v5-final.s11-cap-precheck-recovery-static-ci.v1",
        "validated_exact_commit": subprocess.check_output(
            ["git", "rev-parse", "HEAD"], cwd=ROOT, text=True
        ).strip(),
        "status": "PASS_OUTCOME_FREE_CAP_PRECHECK_RECOVERY_DESIGN",
...
def build_authorization(
    ...
    if (
        report.get("schema") != "v5-final.s11-cap-precheck-recovery-static-ci.v1"
        or report.get("status") != "PASS_OUTCOME_FREE_CAP_PRECHECK_RECOVERY_DESIGN"
        or report.get("decision") != "READY_AWAITING_OWNER_TERMINAL_RECOVERY_AUTHORIZATION"
        ...
        or run_id < 1 or job_id < 1
        or run_url != f"https://github.com/Reimangod/v5-matched-work-study/actions/runs/{run_id}"
    ):
        raise S11CapPrecheckRecoveryError("exact CI evidence is invalid")
    authorization = {
        "schema": "v5-final.s11-cap-precheck-owner-authorization.v1",
        "decision": "GO_EXACT_ITEM_022_ZERO_KERNEL_TERMINAL_RECOVERY_ONLY",
        "owner": "Reimangod",
        "owner_directive": "終わるまで続けて。学術的価値とシステム安全性を常に確認する。",
        "declaration_sha256": _sha(DECLARATION_PATH),
        "declaration_digest": _canonical(DECLARATION_PATH)["declaration_digest"],
        "static_exact_ci": {
            "run_id": run_id,
...
def audit_authorization(*, require_live_incident: bool) -> dict[str, bool]:
    ...
    checks = {
        ...
        == "v5-final.s11-cap-precheck-owner-authorization.v1"
        and authorization.get("decision")
        == "GO_EXACT_ITEM_022_ZERO_KERNEL_TERMINAL_RECOVERY_ONLY"
        and authorization.get("owner") == "Reimangod",
        "declaration_bound": authorization.get("declaration_sha256") == _sha(DECLARATION_PATH)
        and authorization.get("declaration_digest") == _canonical(DECLARATION_PATH)["declaration_digest"],
        "exact_ci_embedded": report.get("schema")
        == "v5-final.s11-cap-precheck-recovery-static-ci.v1"
        and report.get("status") == "PASS_OUTCOME_FREE_CAP_PRECHECK_RECOVERY_DESIGN"
        and all(report.get("checks", {}).values())
        and ci.get("report_sha256") == hashlib.sha256(canonical_json_bytes(report)).hexdigest()
        and ci.get("run_url")
        == f"https://github.com/Reimangod/v5-matched-work-study/actions/runs/{ci.get('run_id')}",
        "scope_exact": authorization.get("authorization") == {
            "append_exact_cap_rejection_event": True,
            "terminalize_existing_checkpoint": True,
            "publish_result_without_molecular_kernel": True,
            "preserve_prior_records": True,
    ...
    if require_live_incident:
        ...
        checks["live_incident_exact"] = (
            ...
            and all(_sha(RAW_LEDGER_ROOT / entry["name"]) == entry["sha256"] for entry in declaration["raw_ledger_prefix"])
            and _sha(CHECKPOINT_PATH) == declaration["outcome_checkpoint_sha256"]
        )
    if not all(checks.values()):
        raise S11CapPrecheckRecoveryError("authorization audit failed")
    return checks
...
def execute_terminal_recovery() -> dict[str, Any]:
    ...
    try:
        recorder._precheck(_projected_binding_delta(), "candidate-generation")
    except ComponentwiseCapRejected:
        pass
    else:
        raise S11CapPrecheckRecoveryError("frozen precheck unexpectedly passed")
    runner.persist_new_work_events(recorder.events)
    after_event = runner.state()
    if (
        len(after_event.records) != len(before.records) + 1
        or len(after_event.work_events) != len(before.work_events) + 1
        ...
        raise S11CapPrecheckRecoveryError("exact zero-delta cap event was not appended")
    ...
    terminal = runner.state(require_terminal=True).terminal
    completion = {
        "schema": "v5-final.s11-cap-precheck-terminal-recovery-completion.v1",
        "queue_index": QUEUE_INDEX,
        "item_key": ITEM_KEY,
        "declaration_sha256": _sha(DECLARATION_PATH),
        "authorization_sha256": _sha(AUTHORIZATION_PATH),
        "records_before": len(before.records),
        "records_after": len(runner.state(require_terminal=True).records),
        "work_total_before": asdict(before.work_total),
        "work_total_after": asdict(runner.state(require_terminal=True).work_total),
...
def main() -> None:
    ...
    if args.write_declaration:
        ...
        if not all((args.ci_report, args.ci_report_sha256, args.run_id, args.job_id, args.run_url)):
            raise S11CapPrecheckRecoveryError("authorization requires exact CI arguments")
