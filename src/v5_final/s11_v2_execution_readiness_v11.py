"""Additive readiness successor after the formal item-028 reconciliation."""
...
from .s11_v2_execution_readiness_v10 import OUTPUT as READINESS_V10
...
from .s11_v2_item028_terminal_reconciliation_v1 import (
...
OUTPUT_DIR = ROOT / "artifacts/v5-final/parent-native/s11-v2-execution-readiness-v11"
OUTPUT = OUTPUT_DIR / "execution-readiness-go-v11.json"
DECISION = "GO_S11_V2_FROZEN_QUEUE_CONTINUATION_FROM_INDEX_29"
SOURCE_PATHS = (
    ...
    "src/v5_final/s11_v2_execution_readiness_v11.py",
    ...
    "tests/test_v5_final_s11_v2_execution_readiness_v11.py",
)
SCOPED_TESTS = tuple(path for path in SOURCE_PATHS if path.startswith("tests/"))


class S11V2ExecutionReadinessV11Error(RuntimeError):
    pass
...
def _require_minimum_free_storage() -> int:
    ...
    if available < MINIMUM_FREE_BYTES:
        raise S11V2ExecutionReadinessV11Error(
...
def inspect_checkpoint() -> dict[str, Any]:
    ...
    predecessor_digests = tuple(
        readiness_v10["accepted_predecessor_receipt_readiness_digests"]
    ...
    next_request = adapter.request(queue["items"][29]["queue_item_id"])
    next_paths = _item_paths(PRODUCTION_ROOT, 29, next_request)
    ...
    checks = {
        ...
        "readiness_v10_is_immutable_historical_predecessor": historical_artifact_valid(
            READINESS_V10,
            readiness_v10,
        ...
        and reconciliation["observed"]["terminal_status"] == "COMPLETED"
        and reconciliation["observed"]["terminal_prefix"] == 29,
        "terminal_prefix_is_exact_29": len(prefix) == 29
        and prefix[-1]["queue_item_id"] == queue["items"][28]["queue_item_id"]
        and prefix[-1]["terminal_status"] == "COMPLETED",
        "item029_is_unique_unstarted_next_item": not any(
    ...
    if failures:
        raise S11V2ExecutionReadinessV11Error(failures)
    ...
    return {
        ...
        "binding": {
            ...
            "readiness_v10": {
                "sha256": _sha(READINESS_V10),
                "readiness_digest": readiness_v10["readiness_digest"],
            },
...
def capture() -> dict[str, Any]:
    if OUTPUT.exists():
        raise S11V2ExecutionReadinessV11Error("readiness v11 artifact already exists")
    branch = _git("branch", "--show-current")
    head = _git("rev-parse", "HEAD")
    if _git("status", "--porcelain"):
        raise S11V2ExecutionReadinessV11Error("capture requires a clean worktree")
    if head != _remote_head(branch):
        raise S11V2ExecutionReadinessV11Error("local and remote heads differ")
    ...
    if not scoped["passed"] or not full["passed"]:
        raise S11V2ExecutionReadinessV11Error("local verification suite failed")
    live = _live_pr_snapshot(expected_head=head)
    available_after_verification = _require_minimum_free_storage()
    body = {
        "schema": "v5-final.s11-v2-execution-readiness.v11",
        "stage": "PHASE_C_POST_ITEM028_TERMINAL_CONTINUATION_READINESS",
        "status": DECISION,
        "decision": DECISION,
        ...
        **evidence,
        "execution_start_index": 29,
        "blockers": [],
        "authorization": {
            "S11_v2_execution": "AUTHORIZED_EXACT_FROZEN_QUEUE_FROM_INDEX_29_ONLY",
        ...
        "semantic_diff": {
            ...
            "item028_disposition": "FORMAL_COMPLETION_AFTER_AUDITED_SAME_ITEM_RETRY",
        },
    }
...
def audit_frozen(*, require_live: bool = False) -> dict[str, Any]:
    ...
    checks = {
        ...
        and bindings["readiness_v10"]["sha256"] == _sha(READINESS_V10)
        and bindings["item028_terminal_reconciliation"]["sha256"]
        ...
        "execution_and_scientific_boundaries": artifact["authorization"][
            "S11_v2_execution"
        ]
