"""Additive readiness successor for the exact item-023 cap-rejection retry."""
...
from .s11_v2_item023_same_item_retry_authorization_v1 import (
    OUTPUT as ITEM023_RETRY_AUTHORIZATION,
...
OUTPUT_DIR = ROOT / "artifacts/v5-final/parent-native/s11-v2-execution-readiness-v8"
OUTPUT = OUTPUT_DIR / "execution-readiness-go-v8.json"
...
SOURCE_PATHS = (
    ...
    "src/v5_final/s11_v2_execution_readiness_v8.py",
...
class S11V2ExecutionReadinessV8Error(RuntimeError):
    pass
...
def _require_minimum_free_storage() -> int:
    ...
    if available < MINIMUM_FREE_BYTES:
        raise S11V2ExecutionReadinessV8Error(
...
def inspect_checkpoint() -> dict[str, Any]:
    ...
    readiness_v7 = _load(READINESS_V7)
    incident = _load(ITEM023_INCIDENT)
    retry = _load(ITEM023_RETRY_AUTHORIZATION)
    predecessor_digests = tuple(
        readiness_v7["accepted_predecessor_receipt_readiness_digests"]
    )
    ...
    request = adapter.request(queue["items"][23]["queue_item_id"])
    paths = _item_paths(PRODUCTION_ROOT, 23, request)
    ...
    checks = {
        ...
        "item023_exactly_rolled_back_nonterminal": replay.terminal is None
        ...
        and retry["observed"]["selected_relation_symbolic_costs"]
        == [5, 5, 5, 10],
        "item023_outcome_services_remain_zero": all(
    ...
    if failures:
        raise S11V2ExecutionReadinessV8Error(failures)
    evidence = _production_evidence()
    return {
        "checks": checks,
        "observed": {
            "terminal_count": len(prefix),
            "item023_status": "ROLLED_BACK_NONTERMINAL",
            "item023_corrected_upper_bound": 452,
            "item023_frozen_symbolic_cap": 447,
            "item023_attempt_1_work_total": {
                key: int(value) for key, value in asdict(replay.work_total).items()
            },
            "item023_attempt_2_expected_new_candidate_energy_evaluations": 0,
            "item023_attempt_2_expected_new_optimizer_starts": 0,
            "item023_attempt_2_expected_new_FCI_evaluations": 0,
            "item023_attempt_2_expected_new_N_dense_expm": 0,
            "FCI_evaluations": 0,
            "N_dense_expm": 0,
        },
        "binding": {
            ...
            "readiness_v7": {
                "sha256": _sha(READINESS_V7),
                "readiness_digest": readiness_v7["readiness_digest"],
            },
            "item023_incident": {
                "sha256": _sha(ITEM023_INCIDENT),
                "incident_digest": incident["incident_digest"],
            },
            "item023_retry_authorization": {
                "sha256": _sha(ITEM023_RETRY_AUTHORIZATION),
...
def capture() -> dict[str, Any]:
    if OUTPUT.exists():
        raise S11V2ExecutionReadinessV8Error("readiness v8 artifact already exists")
    branch = _git("branch", "--show-current")
    head = _git("rev-parse", "HEAD")
    if _git("status", "--porcelain"):
        raise S11V2ExecutionReadinessV8Error("capture requires a clean worktree")
    if head != _remote_head(branch):
        raise S11V2ExecutionReadinessV8Error("local and remote heads differ")
    ...
    if not scoped["passed"] or not full["passed"]:
        raise S11V2ExecutionReadinessV8Error("local verification suite failed")
    live = _live_pr_snapshot(expected_head=head)
    available_after_verification = _require_minimum_free_storage()
    body = {
        "schema": "v5-final.s11-v2-execution-readiness.v8",
        "stage": "PHASE_C_ITEM023_PREVERIFIER_RETRY_READINESS",
        "status": DECISION,
        "decision": DECISION,
        ...
        **evidence,
        ...
        "authorization": {
            "S11_v2_execution": (
                "AUTHORIZED_EXACT_ITEM023_ATTEMPT_2_PREVERIFIER_CAP_REJECTION_ONLY"
            ),
            "item024_and_later": "NOT_AUTHORIZED_PENDING_TERMINAL_RECONCILIATION",
            "candidate_energy": "NOT_AUTHORIZED_FOR_ITEM023_RETRY",
            "optimizer": "NOT_AUTHORIZED_FOR_ITEM023_RETRY",
        ...
        "semantic_diff": {
            ...
            "item023_expected_disposition": "FORMAL_PREVERIFIER_CAP_REJECTION",
        },
    }
...
def audit_frozen(*, require_live: bool = False) -> dict[str, Any]:
    ...
    checks = {
        ...
        and bindings["readiness_v7"]["sha256"] == _sha(READINESS_V7)
        and bindings["item023_incident"]["sha256"] == _sha(ITEM023_INCIDENT)
        and bindings["item023_retry_authorization"]["sha256"]
        == _sha(ITEM023_RETRY_AUTHORIZATION),
        ...
        "only_same_item_retry_open": artifact["authorization"]["S11_v2_execution"]
        == "AUTHORIZED_EXACT_ITEM023_ATTEMPT_2_PREVERIFIER_CAP_REJECTION_ONLY"
        and artifact["authorization"]["item024_and_later"]
        == "NOT_AUTHORIZED_PENDING_TERMINAL_RECONCILIATION",
