"""Additive readiness successor after formal item-023 cap rejection."""
...
from .s11_v2_execution_readiness_v8 import OUTPUT as READINESS_V8
from .s11_v2_execution_runner_v1 import _item_paths, _terminal_prefix
from .s11_v2_item023_terminal_reconciliation_v1 import (
...
OUTPUT_DIR = ROOT / "artifacts/v5-final/parent-native/s11-v2-execution-readiness-v9"
OUTPUT = OUTPUT_DIR / "execution-readiness-go-v9.json"
DECISION = "GO_S11_V2_FROZEN_QUEUE_CONTINUATION_FROM_INDEX_24"
SOURCE_PATHS = (
    ...
    "src/v5_final/s11_v2_execution_readiness_v9.py",
    ...
    "tests/test_v5_final_s11_v2_execution_readiness_v9.py",
)
SCOPED_TESTS = tuple(path for path in SOURCE_PATHS if path.startswith("tests/"))


class S11V2ExecutionReadinessV9Error(RuntimeError):
    pass
...
def _require_minimum_free_storage() -> int:
    ...
    if available < MINIMUM_FREE_BYTES:
        raise S11V2ExecutionReadinessV9Error(
...
def inspect_checkpoint() -> dict[str, Any]:
    ...
    predecessor_digests = tuple(
        readiness_v8["accepted_predecessor_receipt_readiness_digests"]
    ...
    next_request = adapter.request(queue["items"][24]["queue_item_id"])
    next_paths = _item_paths(PRODUCTION_ROOT, 24, next_request)
    ...
    checks = {
        ...
        and reconciliation["observed"]["terminal_prefix"] == 24,
        "terminal_prefix_is_exact_24": len(prefix) == 24
        and prefix[-1]["terminal_status"] == "CAP_REJECTED",
        "item024_is_unique_unstarted_next_item": not any(
    ...
    if failures:
        raise S11V2ExecutionReadinessV9Error(failures)
    ...
    return {
        ...
        "binding": {
            ...
            "readiness_v8": {
                "sha256": _sha(READINESS_V8),
                "readiness_digest": readiness_v8["readiness_digest"],
            },
            "item023_terminal_reconciliation": {
...
def capture() -> dict[str, Any]:
    if OUTPUT.exists():
        raise S11V2ExecutionReadinessV9Error("readiness v9 artifact already exists")
    branch = _git("branch", "--show-current")
    head = _git("rev-parse", "HEAD")
    if _git("status", "--porcelain"):
        raise S11V2ExecutionReadinessV9Error("capture requires a clean worktree")
    if head != _remote_head(branch):
        raise S11V2ExecutionReadinessV9Error("local and remote heads differ")
    ...
    if not scoped["passed"] or not full["passed"]:
        raise S11V2ExecutionReadinessV9Error("local verification suite failed")
    live = _live_pr_snapshot(expected_head=head)
    available_after_verification = _require_minimum_free_storage()
    body = {
        "schema": "v5-final.s11-v2-execution-readiness.v9",
        "stage": "PHASE_C_POST_ITEM023_TERMINAL_CONTINUATION_READINESS",
        "status": DECISION,
        "decision": DECISION,
        ...
        **evidence,
        "execution_start_index": 24,
        "blockers": [],
        "authorization": {
            "S11_v2_execution": "AUTHORIZED_EXACT_FROZEN_QUEUE_FROM_INDEX_24_ONLY",
        ...
        "semantic_diff": {
            ...
            "item023_disposition": "FORMAL_PREVERIFIER_CAP_REJECTION",
        },
    }
...
def audit_frozen(*, require_live: bool = False) -> dict[str, Any]:
    ...
    checks = {
        ...
        and bindings["readiness_v8"]["sha256"] == _sha(READINESS_V8)
        and bindings["item023_terminal_reconciliation"]["sha256"]
